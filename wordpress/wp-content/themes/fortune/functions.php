...
if ( function_exists( 'wp_get_custom_css' ) && class_exists('Kirki') ) {
	  $fields = Kirki::$fields;
		$fields['fortune_theme_options[custom_css]']['description']="This field will no longer work. Since WordPress 4.7 has it's own Custom CSS Editor we recommed you to put all your custom CSS in that field.
		";

		$fields['fortune_theme_options[custom_css]']['tooltip']="This field will no longer work. Since WordPress 4.7 has it's own Custom CSs Editor we recommed you to put all your custom CSS in that field.
		";
		Kirki::$fields = $fields;
}else{
	add_action('wp_head','fortune_custom_css');
	function fortune_custom_css(){
		$fortune_theme_options = fortune_theme_options();
		if($fortune_theme_options['custom_css']!=""){?>
			<style type="text/css"><?php echo ($fortune_theme_options['custom_css']); ?></style>
		<?php }
	}
}
